...
class RetsSession(object):
    def __init__(self, user, passwd, user_agent, user_agent_passwd, rets_version, login_url, proxies=None):
    ...
    def login(self):
        ...
        login_result = self._session.get(self.login_url, proxies=self.proxies)
    ...
    def logout(self):
        ...
        logout_response = self._session.get(logout_url, proxies=self.proxies)
    ...
    def getmetadata(self):
        ...
        if self.user_agent_passwd:
            self._set_rets_ua_authorization()
        response = self._session.get(get_meta_url + '?Type=METADATA-SYSTEM&ID=*&Format=STANDARD-XML', proxies=self.proxies)
    ...
    def _getobject(self, obj_type, resource , obj_id):
        ...
        if self.user_agent_passwd:
            self._set_rets_ua_authorization()
        getobject_response = self._session.get(getobject_url + "?Type=%s&Resource=%s&ID=%s" % (obj_type, resource, obj_id), proxies=self.proxies)
